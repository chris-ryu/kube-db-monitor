...
export default function DeadlocksCard({ deadlocks }) {
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-red-400 text-xs">#{deadlock.transaction_id}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-white/40 text-xs">
                      {new Date(deadlock.created_date).toLocaleTimeString()}
                    </span>
                  </div>
                </div>
                <Button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white font-medium">{selectedDeadlock.transaction_id}</p>
                </div>
                <div>
                  <p className="text-white/60 text-sm">Database</p>
                  <p className="text-white font-medium">{selectedDeadlock.database_name}</p>
                </div>
                <div>
                  <p className="text-white/60 text-sm">Duration</p>
                  <p className="text-white font-medium">{selectedDeadlock.duration}ms</p>
                </div>
                <div>
                  <p className="text-white/60 text-sm">Session</p>
                  <p className="text-white font-medium">{selectedDeadlock.user_session}</p>
                </div>
              </div>
              <div>
                <p className="text-white/60 text-sm mb-2">Query</p>
                <div className="glass-morphism rounded-lg p-3 bg-black/20 border border-white/10">
                  <code className="text-white/80 text-sm">{selectedDeadlock.query}</code>
                </div>
              </div>
            </div>
          )}
