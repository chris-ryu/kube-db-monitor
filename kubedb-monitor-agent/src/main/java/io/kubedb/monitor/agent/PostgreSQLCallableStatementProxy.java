...
/**
 * PostgreSQL CallableStatement 프록시
 * 
 * PostgreSQLPreparedStatementProxy를 확장하여 CallableStatement 특화 기능을 추가합니다.
 * 저장 프로시저 호출 시에도 PostgreSQL 호환성을 보장합니다.
 */
public class PostgreSQLCallableStatementProxy extends PostgreSQLPreparedStatementProxy implements CallableStatement {
    private static final Logger logger = Logger.getLogger(PostgreSQLCallableStatementProxy.class.getName());
    
    private final CallableStatement callableDelegate;
    
    public PostgreSQLCallableStatementProxy(CallableStatement delegate, String sql, 
                                          PostgreSQLCompatibilityHelper compatibilityHelper,
                                          TransactionAwareJDBCInterceptor transactionInterceptor) {
        super(delegate, sql, compatibilityHelper, transactionInterceptor);
        this.callableDelegate = delegate;
    }
    
    // CallableStatement 특화 메서드들
    @Override
    ...
    public void setObject(String parameterName, Object x) throws SQLException {
        // Named parameter도 안전하게 처리
        if (x == null) {
            logger.fine("[KubeDB] PostgreSQL 안전 Named Parameter 바인딩: " + parameterName + " = NULL");
            callableDelegate.setNull(parameterName, Types.VARCHAR);
        } else {
